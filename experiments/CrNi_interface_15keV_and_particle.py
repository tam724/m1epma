...
def make_material_and_parameters(e_beam_position):
    material = physics.Material(
            ...
            hat_n_x = 180,
            hat_n_y = 70,
            dim_x = [e_beam_position-800.*nano, e_beam_position + 800.*nano],
            dim_y = [-800.*nano, 0.]
        )
...
def make_material_and_parameters_small_particle():
    material = physics.Material(
        n_x = 32,
        n_y = 16,
        hat_n_x = 192,
...
def make_experiments_linescan():
    experiments = []
    parameters = []
    for e_beam_position in np.linspace(-56.*(1600./180.)*nano, 56.*(1600./180.)*nano, 56+56+1):
...
def make_experiments_small_particle():
    experiments = []
    material, true_parameter = make_material_and_parameters_small_particle()
...
## TESTING
mat, par = make_material_and_parameters(0.)
...
plt.imshow(test['solution'][0, 0, :, :]); plt.show();
...
x0 = np.full(len(variable_parameters), 0.5) # start value
opt_save.append((x0, 0))
sol = optimization.levenberg_marquard(F, None, None, x0, 20, eps_1=1.e-5, eps_2=1.e-5, f_and_J=f_and_J, callback=callback)
...
## Reconstruction 5
experiments, true_parameters = make_experiments_small_particle()
procs = start_experiment_processes(experiments)
n_k_ratios = sum([e.n_x_ray_transitions for e in experiments])
